...
uint64_t distortion(params_t* params){
    ...
    for (uint64_t i = 0; i < params->npoints; i++){  // For every point
        distortion += params->squared_distance_func(params->points_list+i, params->centroids + params->cluster_id[i], params->dimension);   // Computes the squared distance to its centroid
    }
